...
def train_model(model, train_inputs, train_targets, num_epochs, learning_rate):
    ...
    for epoch in range(num_epochs):
        inputs_tensor = torch.from_numpy(train_inputs).float().unsqueeze(2)
        targets_tensor = torch.from_numpy(train_targets).float().unsqueeze(1)
...
def predict_and_save(model, test_inputs, test_targets, output_file):
    ...
    true_values = []
    with torch.no_grad():
        test_inputs_tensor = torch.from_numpy(test_inputs).float().unsqueeze(2)
